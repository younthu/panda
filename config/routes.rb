...
# 一个可能的原因是require_relative不会加载已经加载过且未更改的文件，每次hotreload的时候engine的routes会被清除
# 但是, require_relative 'routes/api'不会被重新加载，因为这个文件未被改变。所以routes/api.rb里面的routes全丢失了。
#
